    ...
    <div>
      <section className="relative flex min-h-[50vh] items-end overflow-hidden bg-gradient-hero pb-16 pt-32">
        ...
        <div className="container-wide relative">
          <Reveal>
            <span className="eyebrow border-white/30 bg-white/10 text-gold-300">{t("events.eyebrow")}</span>
          </Reveal>
          <Reveal delay={0.1}>
            <h1 className="mt-5 font-display text-4xl font-semibold text-white sm:text-5xl">{t("events.title")}</h1>
          </Reveal>
          <Reveal delay={0.2}>
            <p className="mt-4 max-w-xl text-white/75">{t("events.subtitle")}</p>
          </Reveal>
        </div>
      </section>
            ...
            {loading ? (
              ...
              list.map((event, i) => (
                <Reveal key={event.id} delay={(i % 6) * 0.07}>
                  <button type="button" onClick={() => setSelected(event)} className="block w-full text-left">
      ...
      {selected && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
          ...
          <div className="relative max-h-[88vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white shadow-2xl dark:bg-ink-900">
            ...
            {selected.imageUrl && (
              <FocalImage src={selected.imageUrl} alt={selected.title} className="h-64 w-full object-cover" />
            )}
            <div className="p-8">
              ...
              <div className="mt-4 flex flex-wrap gap-4 text-sm text-ink-900/60 dark:text-cream-100/60">
                <span className="flex items-center gap-1.5">
                  ...
                  {new Date(selected.eventDate || selected.date).toLocaleDateString("en-US", {
                    weekday: "long",
                    month: "long",
                    day: "numeric",
                    year: "numeric",
                ...
                <span className="flex items-center gap-1.5">
                  ...
                  {new Date(selected.eventDate || selected.date).toLocaleTimeString("en-US", {
                    hour: "numeric",
                    minute: "2-digit",
                  })}
                </span>
