...
const SERVICES = [
  { title: "Pirith Chanting", desc: "Protective chanting sessions held for devotees seeking blessings and peace." },
  { title: "Bodhi Pooja", desc: "Weekly offerings at the sacred Bodhi tree, open to the whole community." },
  { title: "Dhamma Sermons", desc: "Monthly sermons from resident and visiting monks on the Buddha's teachings." },
  { title: "Funeral Rites", desc: "Traditional almsgiving and funeral rites conducted with care and dignity." },
];
    ...
    <div>
      <section className="relative flex min-h-[60vh] items-end overflow-hidden bg-gradient-hero pb-16 pt-32">
        ...
        <div className="container-wide relative">
          ...
          <Reveal delay={0.2}>
            <p className="mt-4 max-w-xl text-white/75">{t("temple.subtitle")}</p>
          </Reveal>
        </div>
      </section>
      ...
      <section id="history" className="section-pad scroll-mt-24">
        <div className="container-wide">
          <SectionHeading eyebrow={t("home.historyEyebrow")} title={t("temple.historyTitle")} />
          <div className="mt-14 space-y-16">
            {loading ? (
              <SkeletonCard className="h-72" />
            ) : history.length === 0 ? (
              <p className="text-center text-ink-900/50 dark:text-cream-100/50">{t("common.noData")}</p>
            ) : (
              history.map((item, i) => (
                <Reveal key={item.id}>
                  <div
                  ...
                  >
                    <div className="relative aspect-[4/3] overflow-hidden rounded-3xl shadow-soft">
                      {item.imageUrl ? (
                        <FocalImage src={item.imageUrl} alt={item.title} className="h-full w-full object-cover" />
                      ) : (
      ...
      <section id="chief-monk" className="section-pad scroll-mt-24 bg-cream-100/60 dark:bg-white/[0.02]">
        <div className="container-wide">
          <SectionHeading eyebrow={t("nav.chiefMonk")} title={t("temple.chiefMonkTitle")} />
          <div className="mt-12">
            {loading ? (
          ...
          {residentMonks.length > 0 && (
            <div id="monks" className="scroll-mt-24">
              <div className="mt-20">
                <SectionHeading eyebrow={t("nav.residentMonks")} title={t("temple.monksTitle")} />
              </div>
                  ...
                  <Reveal key={monk.id} delay={i * 0.06}>
                    <div className="flex items-center gap-4 rounded-2xl border border-ink-900/5 bg-white p-5 shadow-card dark:border-white/10 dark:bg-ink-900">
                      ...
                      <div>
                        ...
                        {monk.position && (
                          <p className="text-xs font-medium text-gold-600 dark:text-gold-400">{monk.position}</p>
                        )}
                      </div>
                    </div>
                  </Reveal>
      ...
      <section id="services" className="section-pad scroll-mt-24">
        <div className="container-wide">
          <SectionHeading eyebrow={t("nav.services")} title={t("temple.servicesTitle")} />
            ...
            {SERVICES.map((s, i) => (
              <Reveal key={s.title} delay={i * 0.08}>
                <div className="h-full rounded-2xl border border-ink-900/5 bg-white p-6 shadow-card dark:border-white/10 dark:bg-ink-900">
                  <h3 className="font-display text-lg font-semibold text-ink-900 dark:text-cream-50">{s.title}</h3>
                  <p className="mt-2 text-sm leading-relaxed text-ink-900/55 dark:text-cream-100/55">{s.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
          ...
          <Reveal className="mt-10" delay={0.2}>
            <div className="grid gap-6 sm:grid-cols-2">
              <div className="flex items-center gap-4 rounded-2xl bg-gradient-maroon p-6 text-white shadow-soft">
                ...
                <div>
                  <p className="text-xs uppercase tracking-wide text-white/60">{t("temple.poojaMorning")}</p>
              ...
              <div className="flex items-center gap-4 rounded-2xl bg-gradient-maroon p-6 text-white shadow-soft">
                ...
                <div>
                  <p className="text-xs uppercase tracking-wide text-white/60">{t("temple.poojaEvening")}</p>
                  <p className="font-display text-xl font-semibold">6:30 PM</p>
                </div>
              </div>
            </div>
            <p className="mt-4 text-center text-sm text-ink-900/50 dark:text-cream-100/50">{t("temple.poojaNote")}</p>
          </Reveal>
        </div>
      </section>

      {/* GALLERY */}
      <section id="gallery" className="section-pad scroll-mt-24 bg-cream-100/60 dark:bg-white/[0.02]">
        <div className="container-wide">
          <SectionHeading eyebrow={t("nav.temple_gallery")} title={t("temple.galleryTitle")} />
